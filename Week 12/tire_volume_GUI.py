...
def main():
    ...
    frm_main.pack(padx=40, pady=5, fill=tk.BOTH, expand=1)
...
def populate_main_window(frm_main):
    ...
    lbl_ratio = tk.Label(frm_main, text="Ratio:")
    ...
    # Create a integer entry box where the user will enter her age.
    ent_diameter = numy.FloatEntry(frm_main, 1, 90, width=5)

    # Create a label that displays "Rates:"
    lbl_vol = tk.Label(frm_main, text="Volume (cubic cm):")

    # Create labels that will display the results.
    lbl_results = tk.Label(frm_main, width=8)
    ...
    lbl_ratio.grid(  row=1, column=0, padx=3, pady=3)
    ent_ratio.grid(  row=1, column=1, padx=3, pady=3)
    lbl_diameter.grid(  row=2, column=0, padx=3, pady=3)
    ent_diameter.grid(  row=2, column=1, padx=3, pady=3)
    lbl_vol.grid(row=3, column=0, padx=3, pady=3)
    lbl_results.grid(row=3, column=1, padx=3, pady=3)
    ...
    def calculate(event):
        ...
        try:
            ...
            lbl_results.config(text=f"{volume:.2f}")
